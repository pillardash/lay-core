...
abstract class RequestHelper
{
     ...
     */
    private static array|object $cached_request_form_data;

    /**
     * Cached request files of an already parsed $_FILE data
     * @var array<string, mixed>|object
     */
    private static array|object $cached_request_files;
    ...
    {
        if (isset(self::$cached_request_form_data) and !$invalidate_cache)
            return self::$cached_request_form_data;

        if ($mock_data)
            return self::$cached_request_form_data = $as_array ? $mock_data : (object)$mock_data;
...
//            parse_str($data, $data);
        ...
        self::$cached_request_form_data = json_decode($data, $as_array) ?? $post;

        return self::$cached_request_form_data;
    }

    /**
     * Get $_FILES from the files object, organize it according to request $_FILES name
     * @param bool $as_array
     * @param bool $invalidate_cache
     * @return array|object
     */
    ...
    {
        if (isset(self::$cached_request_files) and !$invalidate_cache)
            return self::$cached_request_files;

        if (empty($_FILES)) return $as_array ? [] : (object)[];

        self::$cached_request_files = $as_array ? LayArray::to_array($_FILES) : LayArray::to_object($_FILES);
