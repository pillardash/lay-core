...
final class SQL
{
    ...
    final public function query(string $query, array $option = []): mixed
    {
        ...
        $is_sqlite = OrmDriver::is_sqlite(self::get_driver());
        ...
        if (empty($query_type)) {
            ...
            $query_type =
                $query_type == OrmQueryType::COUNT->name
                    ? $query_type
                    : strtoupper($qr[0]);
            $query_type =
                LayArray::any(
                    OrmQueryType::cases(),
                    fn($v) => $v->name === $query_type,
                )["value"] ?? $query_type;
        }
        ...
        if ($debug) {
            self::exception(
                ...
                "<pre style='color: #dea303 !important'>$query</pre>
                    <div style='margin: 10px 0'>DB: <span style='color: #00A261'>" .
                    self::$db_name .
                    "</span></div>
                    <div style='margin: 10px 0'>Driver: <span style='color: #00A261'>" .
                    self::$active_driver->value .
                    "</span></div>",
                ["type" => "view"],
            );
        }
        ...
        try {
            ...
            $query_type = is_string($query_type)
                ? $query_type
                : $query_type->name;
        ...
        if ($query_type == OrmQueryType::COUNT) {
            return $this->query_info["data"] =
                (int) (self::$link->fetch_one($exec, OrmReturnType::NUM)[0] ??
                    null);
        }
        ...
        if (!$is_sqlite && $affected_rows == 0) {
            $this->query_info["has_data"] = false;

            if (
                $query_type == OrmQueryType::SELECT ||
                $query_type == OrmQueryType::LAST_INSERTED
            ) {
